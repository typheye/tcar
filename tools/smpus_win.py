...
# ============ UDP闂傚倸鍊搁崐鎼佸磹瀹勬噴褰掑炊椤掑﹦绋忔繝銏ｆ硾椤戝洭銆呴幓鎹楀綊鎮╁顔煎壈缂佺偓鍎冲锟犲蓟閿涘嫪娌悹鍥ㄥ絻婵酣姊?============
...
class ThirdPersonView(QGLWidget):
    ...
    def set_sensor_data(self, pitch, roll, yaw, ax, ay, az):
    ...
    def draw_cube(self):
        ...
        # 闂傚倸鍊搁崐鎼佸磹妞嬪海鐭嗗〒姘ｅ亾妤犵偞鐗犻、鏇㈡晝閳ь剟鎮块濮愪簻闁规媽娉涢惁婊堟煛娴ｇ顏柡灞剧洴椤㈡洟鏁愰崶顏嗏敍闁荤喐绮庢晶妤冩暜閹烘宓侀柕蹇娾偓鑼畾濡炪倖鐗滈崑鐐测枍濠婂懐纾奸柕濞垮労閻撳吋鎱ㄦ繝鍐┿仢鐎规洏鍔嶇换婵嬪礋椤撶姵娈奸梻浣筋嚙鐎涒晠宕欒ぐ鎺戠獥婵娉涚粣? Yaw -> Pitch -> Roll
        # cube_pitch = -roll (闂傚倸鍊搁崐鎼佸磹妞嬪海鐭嗗〒姘ｅ亾妤犵偛顦甸弫鎾绘偐閼碱剦鍞堕梻浣告啞缁哄潡宕曢幎鑺ュ剹闁瑰墽绮悡娆戠磽娴ｉ潧鐏╅柡瀣〒缁辨帡鎮╁畷鍥ь潷婵烇絽娲ら敃顏呬繆閸洖绀嬫い鏃傚帶鐢垶姊绘担鍝ワ紞闁硅櫕鎸剧划鏃堟偨缁嬭锕傛煕閺囥劌鐏犻柛鎰ㄥ亾闂備線娼ц噹闁告洦鍋呴悵顖炴⒒? 缂傚倸鍊搁崐鎼佸磹閹间礁纾归柟闂寸绾惧綊鏌ｉ幋锝呅撻柛濠傛健閺屻劑寮村Δ鈧禍楣冩煕濡ゅ嫰鍝虹紒缁樼箞濡啫鈽夐崡鐐插缂? 闂傚倸鍊搁崐鎼佸磹妞嬪海鐭嗗〒姘ｅ亾妤犵偞鐗犻、鏇㈡晝閳ь剛澹曢悷鎵虫斀闁绘ê鐤囨竟妯肩磼閻橀潧鈻堥柡宀€鍠栭獮鎴﹀箛闂堟稒顔勭紓鍌欒兌婵敻鎮уΔ鍐╁床婵犻潧娲ㄧ弧鈧梺绋挎湰缁矂銆傚ú顏呪拺婵炶尪顕ч獮妤併亜閵娿儻韬€殿喖顭烽崹鎯х暦閸ャ劍顔撴俊鐐€栧濠氬储瑜斿鍐测枎閹惧鍘?
        # cube_roll = pitch (闂傚倸鍊峰ù鍥敋瑜庨〃銉╁传閵壯咁槸婵犵數濮撮崑鍡涙倿娴犲鐓犲┑顔藉姇閳ь剚顨嗙粙澶婎吋婢跺鍘甸柡澶婄墕婢х晫绮婂畡鎳婄懓顭ㄩ崘顏喰ㄩ梺鍝勬湰缁嬫垿鍩ユ径濠庢建闁割偅绻傞～鐘绘⒒娴ｈ棄鍚归柛鐘查閿曘垽鏌嗗鍛枀濠殿喗绻傞惉濂告偪閳ь剙鈹戦悙鏉戠仸闁荤喆鍎甸崺鈧? 缂傚倸鍊搁崐鎼佸磹閹间礁纾归柟闂寸绾惧綊鏌ｉ幋锝呅撻柛濠傛健閺屻劑寮村Δ鈧禍楣冩煕濡ゅ懍鎲鹃柡灞炬礉缁犳稓鈧綆浜栭崑鎾诲即閻樺灚锛?
        ...
        # 婵犵數濮烽弫鍛婃叏閻㈠壊鏁婇柡宥庡幖缁愭淇婇妶鍛殲鐎规洘鐓￠弻鐔兼焽閿曗偓閺嬨倗绱掗埀顒佺節閸嬵垰缍婇弫鎰板川椤撗勵棏闂備胶绮敮鐔肺涢崟顖涚畳闂備胶顭堢换鎰板触鐎ｎ剛绀婇柟瀵稿仒缁诲棙銇勯幇鍓佺Ф闁兼澘娼￠弻鈥崇暆鐎ｎ剛袦闂佽鍠撻崹钘夌暦椤愶箑绀嬮柛顭戝亝閻︽棃姊婚崒娆掑厡妞ゎ厼鐗撻弫鍐Χ婢跺棌鍋撻敃鍌涘€婚柦妯侯槼閹芥洟姊虹紒妯活梿婵炲拑缍侀幆灞解枎閹惧鍘电紓浣割儏濞硷繝顢撳Δ鍐＜闁归偊鍘惧ú瀛樻叏?(濠电姷鏁告慨鐢割敊閺嶎厼绐楁俊銈呭暞閺嗘粍淇婇妶鍛殶闁活厽鐟╅弻鐔衡偓鐢殿焾琚ラ梺绋款儐閹告悂锝炲┑瀣亗閹艰揪绲奸悽鑽ょ磽閸屾瑧鍔嶆慨濠傤煼瀹曚即骞囬钘夊簥濠电娀娼ч鍛矆鐎ｎ偁浜滈柟鎯ь嚟缁犳﹢鏌ｉ悢鍝ョ畺濞ｅ洤锕幃娆擃敂閸曘劌浜鹃柡宥庡幖缁犳澘螖閿濆懎鏆欑痪鎯ь煼閺岀喓鈧數顭堟禒褎銇?
        glDisable(GL_LIGHTING)
...
class MainWindow(QMainWindow):
    ...
    def update_data(self, data):
        ...
        self.gl_view.set_sensor_data(pitch, roll, yaw, ax, ay, az)
