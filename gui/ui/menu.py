...
class MenuManager:
    ...
    def show_main_menu(self):
        """显示主菜单"""
        # menu_items = ["本机", "网络", "显示", "重启"]
        menu_items = ["关于本机", "重启本机"]
        current_index = 0
            ...
            with self.display.lcd.create_canvas() as canvas:
                ...
                if wait_press(KEY_press_PIN) or wait_press(ok_PIN):
                    ...
                    # elif current_index == 3:
                        ...
                        if PowerManager.show_power_off_screen(self.display, self.buttons, self.wifi_manager, self.system_info, self.power_monitor):
                            return
    ...
    def show_selection_menu(self, title, items, vertical=True):
        ...
        while True:
            with self.display.lcd.create_canvas() as canvas:
                ...
                if vertical:
                    current_index = self.buttons.navigate_menu(current_index, len(items), vertical=True)
                else:
                    current_index = self.buttons.navigate_menu(current_index, len(items), vertical=False)
    ...
    def _draw_menu(self, draw, items, current_index):
        ...
        if current_index < visible_items:
            ...
            for i, item in enumerate(items[:visible_items]):
                y_pos = 27 + i * 23
                if i == current_index:
                    Graphics.draw_rounded_rect(draw, 8, y_pos, 219, 19, 18, COLOR_LIGHT_BLUE)
                else:
                    Graphics.draw_rounded_rect(draw, 8, y_pos, 219, 20, 18, COLOR_MID_BLUE)
                draw.text((27, y_pos), item, font=fonts.get_font('normal'), fill=COLOR_WHITE)
        else:
            # 需要滚动显示
            ...
            for i in range(visible_items):
                ...
                draw.text((27, y_pos), items[item_index], font=fonts.get_font('normal'), fill=COLOR_WHITE)
        ...
        while True:
            with self.display.lcd.create_canvas() as canvas:
                ...
                for i, line in enumerate(info_lines):
                    canvas.draw.text((10, 35 + i * 22), line,  # 从35开始，行间距22
    ...
    def show_wifi_connection(self):
        ...
        while True:
            with self.display.lcd.create_canvas() as canvas:
                ...
                # 显示输入字段
                canvas.draw.text((8, 30), "WiFi名称:", font=fonts.get_font('normal'), fill=COLOR_WHITE)
                canvas.draw.text((8, 80), "WiFi密码:", font=fonts.get_font('normal'), fill=COLOR_WHITE)
                ...
                canvas.draw.text((8, 50), ssid_display, font=fonts.get_font('normal'), fill=COLOR_WHITE)
                canvas.draw.text((8, 100), password_display, font=fonts.get_font('normal'), fill=COLOR_WHITE)
        ...
        while True:
            with self.display.lcd.create_canvas() as canvas:
                ...
                if self.system_info:
                    ...
                    ip = ""
                Graphics.draw_header(canvas.draw, ip, self.wifi_manager.get_interfaces())
                Graphics.draw_rounded_rect(canvas.draw, 8, 27, 219, 19, 18, COLOR_MID_BLUE)
                canvas.draw.text((27, 27), "亮度", font=fonts.get_font('normal'), fill=COLOR_WHITE)
                
                # 显示亮度百分比
                percent = temp_level * 10
                canvas.draw.text((127, 27), f"{percent}%", font=fonts.get_font('normal'), fill=COLOR_WHITE)
