...
class Graphics:
    ...
    def signal(draw, left=0, top=0, width=10, height=8, signal=100):
        ...
        draw.polygon([(left, top), (left + tx_w, top), (left + tx_w / 2, top + tx_h / 2),
                      ...
                      (left + tx_w / 2, top)], outline=COLOR_BLUE, fill=COLOR_BLUE)
        for f in range(5):
            if not f * 25 < signal:
                break
            draw.rectangle((
                left + width / 2 + 1 + 3 * f, top + height - height * 0.25 * f, 
                left + width / 2 + 2 + 3 * f, top + height), fill=COLOR_BLUE)

    @staticmethod
    def battery(draw, left=170, top=78, width=50, height=25, battery=100, battery_color = COLOR_BLUE):
        """绘制电池图标"""

        # 电池轮廓 - 厚度3px
        draw.rectangle((left + 1, top, left + width, top + height), 
                    outline=COLOR_MID_BLUE, width = 2, fill=1)

        # 电池正极（右侧凸起）- 在轮廓外留2px空隙，向内移动2px
        draw.rectangle((left + width + 2, top + height / 3,  # 从轮廓右侧开始+2px空隙
                        left + width + 2 + 2, top + height / 3 * 2),  # 宽度1px
                    fill=COLOR_MID_BLUE)
    ...
    @staticmethod
    def car(draw, left=80, top=43, width=80, height=60, status_core=False, status_pyz = False):
        """绘制车图（3:2比例，居中显示）"""
        ...
        draw.rectangle((car_left, 
                        car_top + round(car_inner_height * 0.25), 
                        car_left + car_inner_width, 
                        car_top + round(car_inner_height * 0.25) + round(car_inner_height * 0.5)), 
                    outline=COLOR_WHITE, width=round(car_inner_height * 0.01), fill=COLOR_GREY)
        ...
        # 左上轮
        draw.rectangle((car_left + wheel_inset,
                        car_top,
        ...
        # 右上轮
        draw.rectangle((car_left + car_inner_width - wheel_inset - wheel_width,
                        car_top,
        ...
        # 超声波
        draw.rectangle((car_left + car_inner_width,
                        car_top + round(car_inner_height * 0.35),
                        car_left + car_inner_width + round(car_inner_width * 0.10),
                        car_top + round(car_inner_height * 0.35) + round(car_inner_height * 0.3)),
                    outline=COLOR_WHITE, width=round(car_inner_height * 0.01), fill=COLOR_MID_BLUE)
            
        
        # 主机
        draw.rectangle((car_left + round(car_inner_height * 0.07), 
                        car_top + round(car_inner_height * 0.35), 
                        car_left + round(car_inner_height * 0.07) + round(car_inner_height * 0.3), 
                        car_top + round(car_inner_height * 0.35) + round(car_inner_height * 0.3)),
                        outline=COLOR_MID_BLUE, width=round(car_inner_height * 0.02), fill=COLOR_GREEN if status_core else COLOR_YELLOW)
        
        # 分割线
        draw.rectangle((car_left + round(car_inner_height * (0.08 * 2 + 0.29)), 
                        car_top + round(car_inner_height * 0.25), 
                        car_left + round(car_inner_height * (0.08 * 2 + 0.29)) + round(car_inner_height * 0.15), 
                        car_top + round(car_inner_height * 0.25) + round(car_inner_height * 0.5)), 
                        outline=COLOR_WHITE, width=round(car_inner_height * 0.01), fill=COLOR_WHITE)
        
        # 云台
        draw.rectangle((car_left + round(car_inner_height * (0.08 * 2 + 0.51)), 
                        car_top + int(car_inner_height * 0.35), 
                        car_left + round(car_inner_height * (0.08 * 2 + 0.51)) + round(car_inner_height * 0.3) , 
                        car_top + int(car_inner_height * 0.35) + round(car_inner_height * 0.3)),
                        outline=COLOR_MID_BLUE, width=round(car_inner_height * 0.02), fill=COLOR_GREEN if status_pyz else COLOR_YELLOW)
    ...
    def wifi(draw, left=0, top=0, width=9, height=9, signal=100, num=1):
        ...
        if signal > 0:
            ...
            draw.pieslice((left + 3, top + 3, left + width * 2 - 3, top + height * 2 - 3), -135, -45, fill=COLOR_DARK_BLUE)
            draw.pieslice((left + 6, top + 6, left + width * 2 - 6, top + height * 2 - 6), -135, -45, fill=blue)
            draw.pieslice((left + 9, top + 9, left + width * 2 - 9, top + height * 2 - 9), -135, -45, fill=COLOR_DARK_BLUE)
            draw.pieslice((left + 12, top + 12, left + width * 2 - 12, top + height * 2 - 12), -135, -45, fill=blue)
        else:
            draw.pieslice((left, top, left + width * 2, top + height * 2), -135, -45, outline=gray, fill=gray)
        
        draw.text((left + 24, top + 8), "%s" % num, font=fonts.get_font('small'), fill=COLOR_BLUE)

    @staticmethod
    def draw_header(draw, ip="", wifi_status={}):
        """绘制页面头部信息"""
        if ip:
        ...
        count = 0
        for iface, status in wifi_status.items():
            if status:
                count += 1
    ...
    def draw_footer(draw, left_text="确定", right_text="返回", center=""):
        ...
        if left_text:
            draw.text((2, footer_y), left_text, font=fonts.get_font('normal'), fill=COLOR_WHITE)
        if right_text:
            draw.text((SCREEN_WIDTH - font_width * len(right_text) - 2, footer_y), 
                     right_text, font=fonts.get_font('normal'), fill=COLOR_WHITE)
        if center:
            draw.text((SCREEN_WIDTH / 2 - font_width * len(center) / 2, footer_y), 
                     center, font=fonts.get_font('normal'), fill=COLOR_WHITE)
    ...
    def draw_background(draw, title):
        ...
        line_width = 2
        draw.polygon(
            [(0, 0), (90, 0), (110, 20), (215, 20), (235, 40), 
             (235, 215), (20, 215), (0, 195), (0, 0)],
            fill=COLOR_BLUE)
        ...
        draw.polygon(
            [(line_width, line_width),
             ...
             (line_width, line_width)],
            outline=COLOR_BLUE, fill=COLOR_DARK_BLUE)
        ...
        draw.polygon([(8, 8), (16, 8), (16, 18), (8, 18), (8, 16), 
                     (14, 14), (14, 12), (8, 10), (14, 10), (14, 8)],
        ...
        # 时间显示
        import time
        draw.text((193, 1), time.strftime("%H:%M", time.localtime()), 
                 font=fonts.get_font('header'), fill=COLOR_WHITE)

    @staticmethod
    def draw_rounded_rect(draw, left=5, top=20, width=0, height=0, radius=10, color=COLOR_MID_BLUE):
        """绘制圆角矩形区域"""
        draw.polygon(
            [(left, top), 
