...
--speed 20"'''
    ...
    def update_dynamic_adjustment(self, frame_rgb):
        ...
        if (self.frames_displayed % 30 == 0 or 
            current_time - self.last_change_check_time > 2.0):
            ...
            if frame_rgb is not None:
                if not self.should_display_frame():
                    ...
                    try:
                        ...
                        log(2, "相机显示错误:", str(e))
                        self.show_ui, self.show_ui_msg, self.show_ui_disable = 1, "相机显示错误", False
                        self.close_camera()
                        break
            else:
                self.show_ui, self.show_ui_msg, self.show_ui_disable = 1, "相机无信号", False
                self.close_camera()
                break
        ...
        while True:
            if self.show_ui == 0:
                ...
                with self.display.lcd.create_canvas() as canvas:
                    ...
                    Graphics.draw_footer(canvas.draw, left_text="重置", center="相机")
                    ...
                    if wait_press(main_PIN):
                        ...
                        if not self.camera_enabled:
                            if get_core_status():
                                self.show_ui, self.show_ui_msg, self.show_ui_disable = 2, "正在连接相机...", True
                            else:
                                self.show_ui, self.show_ui_msg, self.show_ui_disable = 1, "中枢主机未连接", False
                        else:
                            self.close_camera()
